...
class SideMenuBody extends StatelessWidget {
          ...
          Expanded(
            child: _buildAnimatedPart(
              ...
              child: ListView.separated(
                ...
                separatorBuilder: (context, index) => SizedBox(height: data.itemSpacing),
              ),
            ),
          ),
  ...
  Widget _buildSideMenuItem(SideMenuItemData item) {
    return switch (item) {
      SideMenuItemDataTile() => SideMenuItemTile(
          minWidth: minWidth,
          isOpen: isOpen,
          data: item.resolveWith(g: data.defaultTileData),
        ),
      SideMenuItemDataTitle() => SideMenuItemTitle(
          data: item,
          minWidth: minWidth,
          isOpen: isOpen,
        ),
      SideMenuItemDataDivider() => SideMenuItemDivider(
          data: item,
        ),
      SideMenuItemDataExpandable() => SideMenuItemExpandable(
          minWidth: minWidth,
          isOpen: isOpen,
          data: item.resolveWith(g: data.defaultTileData),
        ),
